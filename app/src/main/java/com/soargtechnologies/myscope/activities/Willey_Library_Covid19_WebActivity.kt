...
class Willey_Library_Covid19_WebActivity :BaseActivity() {

    companion object {
        const val PAGE_URL = "pageUrl_Wiley_Library"
        const val MAX_PROGRESS = 100
        fun newIntent1(context: Context, pageUrl_Wiley_Library: String): Intent {
            val intent = Intent(context, Willey_Library_Covid19_WebActivity::class.java)
            intent.putExtra(PAGE_URL, pageUrl_Wiley_Library)
            return intent
        }
    }
    private lateinit var pageUrl_Wiley_Library: String
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activitiesToolbar()
        header1!!.text = "Wiley Online Library"
...
//        webView1.loadUrl("http://www.covid19india.org/")
        pageUrl_Wiley_Library = intent.getStringExtra(PAGE_URL)
                ?: throw IllegalStateException("field $PAGE_URL missing in Intent")
        initWebView()
        setWebClient()
        handlePullToRefresh()
        loadUrl(pageUrl_Wiley_Library)
    }
    ...
    private fun loadUrl(pageUrl_Wiley_Library: String) {
        webView1.loadUrl(pageUrl_Wiley_Library)
    }
