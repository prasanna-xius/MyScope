...
class Medical_Journey_Covid19_WebActivity :BaseActivity(){
    companion object {
        const val PAGE_URL = "pageUrl_Covid_British_Medical"
        const val MAX_PROGRESS = 100
        fun newIntent1(context: Context, pageUrl_Covid_British_Medical: String): Intent {
            val intent = Intent(context, Medical_Journey_Covid19_WebActivity::class.java)
            intent.putExtra(PAGE_URL, pageUrl_Covid_British_Medical)
            return intent
        }
    }
    private lateinit var pageUrl_Covid_British_Medical: String
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activitiesToolbar()
        header1!!.text = "British Medical Journal"
...
//        webView1.loadUrl("http://www.covid19india.org/")
        pageUrl_Covid_British_Medical = intent.getStringExtra(PAGE_URL)
                ?: throw IllegalStateException("field $PAGE_URL missing in Intent")
        initWebView()
        setWebClient()
        handlePullToRefresh()
        loadUrl(pageUrl_Covid_British_Medical)
    }
    ...
    private fun loadUrl(pageUrl_Covid_British_Medical: String) {
        webView1.loadUrl(pageUrl_Covid_British_Medical)
    }
