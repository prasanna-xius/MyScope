...
import com.soargtechnologies.myscope.helpers.Document_ImmunAdapter
...
class MedicalDoc_Immun : BaseActivity() {
    var file: File? = null
    private var mImageUrl = ""
    var p_uploadid: Int = 0
    var recyclerView: RecyclerView? = null
    var DocAdapter: Document_ImmunAdapter? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        docimmun.setProgressBackgroundColorSchemeColor(ContextCompat.getColor(this, R.color.colorPrimary))
        docimmun.setColorSchemeColors(Color.WHITE)
        docimmun.setOnRefreshListener {
...
//                loadDestinations()
        ...
        val fab = findViewById<View>(R.id.fab_immundocs) as FloatingActionButton
            ...
            override fun onResponse(call: Call<MutableList<PrescriptionDataClass>>, response: Response<MutableList<PrescriptionDataClass>>) {
                if (response.isSuccessful()) {
                    ...
                    val imageList = response.body()!!
                     DocAdapter= Document_ImmunAdapter(imageList)
                    recyclerView!!.adapter = DocAdapter
                    immundoc_recycler_view.adapter?.notifyDataSetChanged()
    ...
    private fun uploadImage(imageBytes: ByteArray, code: Int) {
        ...
        if (codevalue.equals(102)) {
            ...
            body = MultipartBody.Part.createFormData("document_immunization_file", "Rx", requestFile)
            ...
            body = MultipartBody.Part.createFormData("document_immunization_file", "image.jpg", requestFile)
            type = RequestBody.create(MediaType.parse("text/plain"), "image")

        }
        val call = destinationService.uploaddocImmun( mobile,body, date,type)
