...
class Elsevier_Network_Covid19_WebActivity :BaseActivity(){
    companion object {
        const val PAGE_URL = "pageUrl_Elsevier"
        const val MAX_PROGRESS = 100
        fun newIntent1(context: Context, pageUrl_Elsevier: String): Intent {
            val intent = Intent(context, Elsevier_Network_Covid19_WebActivity::class.java)
            intent.putExtra(PAGE_URL, pageUrl_Elsevier)
            return intent
        }
    }
    private lateinit var pageUrl_Elsevier: String
...
//        webView1.loadUrl("http://www.covid19india.org/")
        pageUrl_Elsevier = intent.getStringExtra(PAGE_URL)
                ?: throw IllegalStateException("field $PAGE_URL missing in Intent")
        initWebView()
        setWebClient()
        handlePullToRefresh()
        loadUrl(pageUrl_Elsevier)
    }
    ...
    private fun loadUrl(pageUrl_Elsevier: String) {
        webView1.loadUrl(pageUrl_Elsevier)
    }
