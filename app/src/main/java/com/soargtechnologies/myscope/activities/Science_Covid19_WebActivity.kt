...
class Science_Covid19_WebActivity:BaseActivity() {

    companion object {
        const val PAGE_URL = "pageUrl_Covid_Sciencemag"
        const val MAX_PROGRESS = 100
        fun newIntent1(context: Context, pageUrl_Covid_Sciencemag: String): Intent {
            val intent = Intent(context, Science_Covid19_WebActivity::class.java)
            intent.putExtra(PAGE_URL, pageUrl_Covid_Sciencemag)
            return intent
        }
    }
    private lateinit var pageUrl_Covid_Sciencemag: String
...
//        webView1.loadUrl("http://www.covid19india.org/")
        pageUrl_Covid_Sciencemag = intent.getStringExtra(PAGE_URL)
                ?: throw IllegalStateException("field $PAGE_URL missing in Intent")
        initWebView()
        setWebClient()
        handlePullToRefresh()
        loadUrl(pageUrl_Covid_Sciencemag)
    }
    ...
    private fun loadUrl(pageUrl_Covid_Sciencemag: String) {
        webView1.loadUrl(pageUrl_Covid_Sciencemag)
    }
