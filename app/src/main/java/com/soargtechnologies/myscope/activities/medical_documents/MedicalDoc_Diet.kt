...
import com.soargtechnologies.myscope.helpers.Document_DietAdapter
...
class MedicalDoc_Diet : BaseActivity() {
    var file: File? = null
    private var mImageUrl = ""
    var p_uploadid: Int = 0
    var recyclerView: RecyclerView? = null
    var DocAdapter: Document_DietAdapter? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_medical_doc__diet)
        ...
        docdiet.setProgressBackgroundColorSchemeColor(ContextCompat.getColor(this, R.color.colorPrimary))
        docdiet.setColorSchemeColors(Color.WHITE)
        docdiet.setOnRefreshListener {
        ...
        val fab = findViewById<View>(R.id.fab_dietdocs) as FloatingActionButton
    ...
    private fun loadDestinations() {
        ...
        val requestCall = service.getDietDetails("8142529582")
            ...
            override fun onResponse(call: Call<MutableList<PrescriptionDataClass>>, response: Response<MutableList<PrescriptionDataClass>>) {
                if (response.isSuccessful()) {
                    ...
                    val imageList = response.body()!!
                     DocAdapter= Document_DietAdapter(imageList)
                    recyclerView!!.adapter = DocAdapter
                    diet_recycler_view.adapter?.notifyDataSetChanged()
    ...
    private fun uploadImage(imageBytes: ByteArray, code: Int) {
        ...
        if (codevalue.equals(102)) {
            ...
            body = MultipartBody.Part.createFormData("document_diet_file", "Rx", requestFile)
            ...
            body = MultipartBody.Part.createFormData("document_diet_file", "image.jpg", requestFile)
            type = RequestBody.create(MediaType.parse("text/plain"), "image")

        }
        val call = destinationService.uploaddocDiet( mobile,body, date,type)
