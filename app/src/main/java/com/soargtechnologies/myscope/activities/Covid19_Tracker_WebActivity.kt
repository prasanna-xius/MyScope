...
class Covid19_Tracker_WebActivity :BaseActivity() {
    ...
    companion object {
        const val PAGE_URL = "pageUrl_Covidtracker"
        const val MAX_PROGRESS = 100
        fun newIntent(context: Context, pageUrl_Covidtracker: String): Intent {
            val intent = Intent(context, Covid19_Tracker_WebActivity::class.java)
            intent.putExtra(PAGE_URL, pageUrl_Covidtracker)
            return intent
        }
    }
    private lateinit var pageUrl_Covidtracker: String
...
//        webView1.loadUrl("http://www.covid19india.org/")
        pageUrl_Covidtracker = intent.getStringExtra(PAGE_URL)
                ?: throw IllegalStateException("field $PAGE_URL missing in Intent")
        initWebView()
        setWebClient()
        handlePullToRefresh()
        loadUrl(pageUrl_Covidtracker)
    }
