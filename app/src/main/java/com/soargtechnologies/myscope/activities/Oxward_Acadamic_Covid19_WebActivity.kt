...
class Oxward_Acadamic_Covid19_WebActivity :BaseActivity() {

    companion object {
        const val PAGE_URL = "pageUrl_Covid_Oxward_Acadamic"
        const val MAX_PROGRESS = 100
        fun newIntent1(context: Context, pageUrl_Covid_Oxward_Acadamic: String): Intent {
            val intent = Intent(context, Oxward_Acadamic_Covid19_WebActivity::class.java)
            intent.putExtra(PAGE_URL, pageUrl_Covid_Oxward_Acadamic)
            return intent
        }
    }
    private lateinit var pageUrl_Covid_Oxward_Acadamic: String
...
//        webView1.loadUrl("http://www.covid19india.org/")
        pageUrl_Covid_Oxward_Acadamic = intent.getStringExtra(PAGE_URL)
                ?: throw IllegalStateException("field $PAGE_URL missing in Intent")
        initWebView()
        setWebClient()
        handlePullToRefresh()
        loadUrl(pageUrl_Covid_Oxward_Acadamic)
    }
    ...
    private fun loadUrl(pageUrl_Covid_Oxward_Acadamic: String) {
        webView1.loadUrl(pageUrl_Covid_Oxward_Acadamic)
    }
