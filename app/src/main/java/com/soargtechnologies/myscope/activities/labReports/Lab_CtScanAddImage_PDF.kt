...
import com.soargtechnologies.myscope.helpers.Lab_CtScan_ImageAdapter
...
import kotlinx.android.synthetic.main.activity_lab_ctscan_image_list.*
...
class Lab_CtScanAddImage_PDF : AppCompatActivity() {
    var file: File? = null
    var uri: Uri? = null
    ...
    var labAdapter: Lab_CtScan_ImageAdapter? = null
   ...
   // var   carItemList:List<PrescriptionDataClass>? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_lab_ctscan_image_list)
       ...
       // model_name = sharedpreferences!!.getString("model_name",null)!!
        ...
        recyclerView = findViewById(R.id.lab_ctscan_recycler_view)
    ...
    private fun loadDestinations() {
        ...
        val requestCall = service.getCtscanDetails()
        ...
        requestCall.enqueue(object : Callback<MutableList<Lab_ReportDataClass>> {
            ...
            override fun onResponse(call: Call<MutableList<Lab_ReportDataClass>>, response: Response<MutableList<Lab_ReportDataClass>>) {
                if (response.isSuccessful()) {
                    ...
                    labAdapter = Lab_CtScan_ImageAdapter(imageList!!)
                    recyclerView!!.adapter = labAdapter

                    lab_ctscan_recycler_view.adapter?.notifyDataSetChanged()
                } else if (response.code() == 401) {
                    Toast.makeText(this@Lab_CtScanAddImage_PDF,
                            "Your session has expired. Please Login again.", Toast.LENGTH_LONG).show()
                } else {
                    // Application-level failure
                    // Your status code is in the range of 300's, 400's and 500's
                    Toast.makeText(this@Lab_CtScanAddImage_PDF, "Failed to retrieve items", Toast.LENGTH_LONG).show()
                }
            }
            ...
            override fun onFailure(call: Call<MutableList<Lab_ReportDataClass>>, t: Throwable) {
                ...
                Toast.makeText(this@Lab_CtScanAddImage_PDF, "Error Occurred" + t.toString(), Toast.LENGTH_LONG).show()
            }
    ...
    private fun uploadImage(imageBytes: ByteArray, code: Int, uri: Uri?) {
       ...
       // var model = RequestBody.create(MediaType.parse("text/plain"), model_name)
        ...
        if (codevalue.equals(102)) {
            ...
            body = MultipartBody.Part.createFormData("lab_ctscan_file", "Rx", requestFile)
            ...
            body = MultipartBody.Part.createFormData("lab_ctscan_file", "image.jpg", requestFile)
            type = RequestBody.create(MediaType.parse("text/plain"), "image")

        }
        val call = destinationService.uploadCtscanDetails(body, mobile_no, date,type)
...
//                        val errorResponse = gson.fromJson<PrescriptionDataClass>(errorBody!!.string(), Response::class.java!!)
    ...
    companion object {
        val TAG = Lab_CtScanAddImage_PDF::class.java!!.getSimpleName()
