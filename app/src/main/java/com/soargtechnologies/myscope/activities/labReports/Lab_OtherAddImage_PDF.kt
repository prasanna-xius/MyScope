...
import com.soargtechnologies.myscope.helpers.Lab_Others_ImageAdapter
...
import kotlinx.android.synthetic.main.activity_lab_others_image_list.*
...
class Lab_OtherAddImage_PDF : AppCompatActivity() {
    var file: File? = null
    var uri: Uri? = null
    ...
    var labAdapter: Lab_Others_ImageAdapter? = null
   ...
   // var   carItemList:List<PrescriptionDataClass>? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_lab_others_image_list)
       ...
       // model_name = sharedpreferences!!.getString("model_name",null)!!
        ...
        recyclerView = findViewById(R.id.lab_others_recycler_view)
    ...
    private fun loadDestinations() {
        ...
        val requestCall = service.getOthersDetails()
        ...
        requestCall.enqueue(object : Callback<MutableList<Lab_ReportDataClass>> {
            ...
            override fun onResponse(call: Call<MutableList<Lab_ReportDataClass>>, response: Response<MutableList<Lab_ReportDataClass>>) {
                if (response.isSuccessful()) {
                    ...
                    labAdapter = Lab_Others_ImageAdapter(imageList!!)
                    recyclerView!!.adapter = labAdapter

                    lab_others_recycler_view.adapter?.notifyDataSetChanged()

                } else if (response.code() == 401) {
                    Toast.makeText(this@Lab_OtherAddImage_PDF,
                            "Your session has expired. Please Login again.", Toast.LENGTH_LONG).show()
                } else {
                    // Application-level failure
                    // Your status code is in the range of 300's, 400's and 500's
                    Toast.makeText(this@Lab_OtherAddImage_PDF, "Failed to retrieve items", Toast.LENGTH_LONG).show()
                }
            }
            ...
            override fun onFailure(call: Call<MutableList<Lab_ReportDataClass>>, t: Throwable) {
                ...
                Toast.makeText(this@Lab_OtherAddImage_PDF, "Error Occurred" + t.toString(), Toast.LENGTH_LONG).show()
            }
    ...
    private fun uploadImage(imageBytes: ByteArray, code: Int, uri: Uri?) {
       ...
       // var model = RequestBody.create(MediaType.parse("text/plain"), model_name)
        ...
        if (codevalue.equals(102)) {
            ...
            body = MultipartBody.Part.createFormData("lab_others_file", "Lr", requestFile)
            ...
            body = MultipartBody.Part.createFormData("lab_others_file", "image.jpg", requestFile)
            type = RequestBody.create(MediaType.parse("text/plain"), "image")

        }
        val call = destinationService.uploadOthersDetails(body, mobile_no, date,type)
...
//                        val errorResponse = gson.fromJson<PrescriptionDataClass>(errorBody!!.string(), Response::class.java!!)
    ...
    companion object {
        val TAG = Lab_OtherAddImage_PDF::class.java!!.getSimpleName()
