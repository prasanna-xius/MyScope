...
class CDC_Centers_Covid19_WebActivity :BaseActivity(){
    companion object {
        const val PAGE_URL = "pageUrl_Centers_Cdc"
        const val MAX_PROGRESS = 100
        fun newIntent1(context: Context, pageUrl_Centers_Cdc: String): Intent {
            val intent = Intent(context, CDC_Centers_Covid19_WebActivity::class.java)
            intent.putExtra(PAGE_URL, pageUrl_Centers_Cdc)
            return intent
        }
    }
    private lateinit var pageUrl_Centers_Cdc: String
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activitiesToolbar()
        header1!!.text = "Centers for Disease Control and Prevention"

//        webView.loadUrl("https://coronavirus.jhu.edu/map.html")

        pageUrl_Centers_Cdc = intent.getStringExtra(PAGE_URL)
                ?: throw IllegalStateException("field ${PAGE_URL} missing in Intent")
        initWebView()
        setWebClient()
        handlePullToRefresh()
        loadUrl(pageUrl_Centers_Cdc)
    }
    ...
    private fun loadUrl(pageUrl_Centers_Cdc: String) {
        webView.loadUrl(pageUrl_Centers_Cdc)
